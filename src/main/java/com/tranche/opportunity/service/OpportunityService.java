...
public class OpportunityService {
    ...
    @Caching(evict = {
            @CacheEvict(cacheNames = OpportunityCacheNames.LIVE_LISTINGS, allEntries = true),
            @CacheEvict(cacheNames = OpportunityCacheNames.DETAIL, key = "'v2:' + #id")
    })
    ...
    @Cacheable(
            ...
            key = "'v2:' + #riskGrade + ':' + #pageable.pageNumber + ':' + #pageable.pageSize + ':' + #pageable.sort"
    )
    ...
    @Transactional(readOnly = true)
    @Cacheable(cacheNames = OpportunityCacheNames.DETAIL, key = "'v2:' + #id")
